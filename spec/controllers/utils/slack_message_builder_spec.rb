...
describe SlackMessageBuilder do
  describe '#build' do
    it 'returns the full message' do
      ...
      *Request:*
      > <@Alice> estimates that their contribution grows the pie by *+$5.0* for the *5.0 hours* spent
      *Description:*
      > first line
      > second line
      *Requested Changes:*
    ...
    describe '#request_body_better' do
      it 'returns a custom message with a single contributor' do
        ...
        expect(text).to eql <<~SLACK_TEMPLATE
          *Request:*
          > <@Bob> estimates that their contribution grows the pie by *+$5.0* for the *5.0 hours* spent
        SLACK_TEMPLATE
      end
      ...
      it 'returns a custom message with a single contributor' do
        ...
        expect(text).to eql <<~SLACK_TEMPLATE
          *Request:*
          > <@Alice> estimates that <@Bob>'s contribution grows the pie by *+$5.0* for the *5.0 hours* spent
        SLACK_TEMPLATE
      end
      ...
      it 'returns a custom message with many contributors' do
        ...
        expect(text).to eql <<~SLACK_TEMPLATE
          *Request:*
          > <@Bob> estimates that <@Alice> and <@Bob>'s contribution grows the pie by *+$15.0* for the *15.0 person-hours* spent
        SLACK_TEMPLATE
      end
      ...
      it 'returns a custom message with many contributors' do
        ...
        expect(text).to eql <<~SLACK_TEMPLATE
          *Request:*
          > <@Bob> estimates that <@Alice>, <@Bob>, and <@Mike>'s contribution grows the pie by *+$16.0* for the *16.0 person-hours* spent
        SLACK_TEMPLATE
      end
    end
